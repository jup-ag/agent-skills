#!/usr/bin/env tsx
/**
 * execute-trigger.ts - Execute Trigger orders on Jupiter
 *
 * Usage:
 *   pnpm execute-trigger --request-id "UUID" --signed-tx "BASE64_SIGNED_TX"
 */
...
const EXECUTE_URL = "https://api.jup.ag/trigger/v1/execute";
...
function isExecuteResponse(value: unknown): value is ExecuteResponse {
  ...
  if ("error" in data && data.error !== undefined && typeof data.error !== "string") {
    return false;
  }

  if ("code" in data && data.code !== undefined && typeof data.code !== "number") {
    return false;
  }
...
async function executeOrder(options: ExecuteOptions): Promise<void> {
  ...
  if (!options.requestId) {
    fail("--request-id is required", [
      "Get the requestId from the /trigger/v1/createOrder response.",
    ]);
  }
  ...
  try {
    ...
    const operation = "POST /trigger/v1/execute";
    ...
    if (!isExecuteResponse(parsed)) {
      fail("Unexpected /trigger/v1/execute response shape.", [
    ...
    if (data.status === "Success" && data.signature) {
      ...
      fail("Transaction failed during Trigger execute.", details);
    }
  } catch (error) {
    ...
    if (error instanceof Error) {
      if (error.name === "AbortError") {
        fail(`Request timed out after ${REQUEST_TIMEOUT_MS / 1000} seconds`, [
          "The order MAY have been placed on-chain despite the timeout.",
...
program
  ...
  .description("Execute Trigger orders on Jupiter")
  .requiredOption(
    "-r, --request-id <id>",
    "Request ID from /trigger/v1/createOrder response"
  )
  .requiredOption(
    "-t, --signed-tx <base64>",
